...
public class ApiResource {
    ...
    private OrderService orderService = ApplicationContext.INSTANCE.getOrderService();
    ...
    @GET
    @Path("categoriesTopThree")
    @Produces(MediaType.APPLICATION_JSON)
    public List<Category> categoriesGetTopThree(@Context HttpServletRequest httpRequest, @QueryParam("limit") @DefaultValue("3") int limit) {
        try {
            return categoryDao.findTopThree(limit);
        } catch (Exception e) {
            throw new ApiException("categories lookup failed", e);
        }
    }
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public Category categoriesByName(@PathParam("category-name") String categoryName,
                         @Context HttpServletRequest httpRequest) {
        try {
            Category category = categoryDao.findByName(categoryName);
            if (category == null) {
                throw new ApiException(String.format("No such category name: %n", categoryName));
            }
            return category;
        } catch (Exception e) {
            throw new ApiException(String.format("Categories lookup by category-name %n failed", categoryName), e);
        }
    }
    ...
    public List<Book> booksByCategoryName(@PathParam("category-name") String categoryName,
        ...
        try {
            ...
            throw new ApiException(String.format("Book lookup by category-name %n failed", categoryName), e);
        }
    }
